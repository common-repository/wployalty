...
class WlrBirthdayEmail extends \WC_Email
{
    ...
    protected function sendBirthdayEmail($email,$reward,$action_type,$data,$campaign,$type = 'point'){
        if (!empty($email) && (($type == 'point' && $reward > 0)|| ($type == 'coupon' && !empty($reward)))){
            ...
            if (!$is_send_email || $is_banned_user  || !apply_filters('wlr_before_send_email',true,array(
                'email_type'=>$this->id,'reward'=>$reward,'action_type'=>$action_type,'data'=>$data,'campaign'=>$campaign,'type'=>$type))){
                return;
            }
            ...
            if ($type == 'coupon' && !empty($reward) && isset($reward->display_name) && !empty($reward->display_name)) {
                ...
                $reward_title = isset($reward->name) && !empty($reward->name) ? $reward->name: '';
            }
            $earn_reward_coupon = '';
            ...
            $short_codes = array(
                '{wlr_reward_title}' => $reward_title,
                '{wlr_points_label}' => $point_label,
                '{wlr_reward_label}' => $reward_label,
                '{wlr_reward_display_name}' => $display_name,
                ...
                '{wlr_earn_reward}' => !empty($earn_reward_coupon) ? $earn_reward_coupon :$display_name,
            ...
            $short_codes = apply_filters('wlr_earn_birthday_email_short_codes', $short_codes,$email,$reward,$action_type,$data,$campaign,$type);
