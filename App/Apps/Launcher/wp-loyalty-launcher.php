 ...
 */
if ( ! function_exists( 'isWployaltyActiveOrNotInLauncher' ) ) {
	function isWployaltyActiveOrNotInLauncher() {
...
if ( isWployaltyActiveOrNotInLauncher() ) {
	/**
	 * Plugin Constants
	 */
	...
	if ( class_exists( \Wll\V2\App\Router::class ) ) {
		...
		if ( method_exists( $router, 'initHooks' ) ) {
			$router->initHooks();
		}
	}
}
