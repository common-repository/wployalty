...
class Main extends Base {
	...
	function addMenu() {
		if ( Woocommerce::hasAdminPrivilege() ) {
			add_menu_page( __( 'WPLoyalty', 'wp-loyalty-rules' ), __( 'WPLoyalty', 'wp-loyalty-rules' ), 'manage_woocommerce', WLR_PLUGIN_SLUG, array(
				$this,
				'manageLoyaltyPages'
	...
	function adminScripts( $hook ) {
		...
		foreach ( $js_files as $file ) {
			...
			if ( $js_file_name == 'main.bundle.js' ) {
				...
				wp_register_script( $js_name, $js_file_url, array(
					'jquery',
	...
	function enableMyRewardSection() {
		...
		if ( $is_enabled === 'yes' ) {
			$files   = apply_filters( 'wlr_customer_reward_page_templates', array(
				'customer_page.php',
				'cart_page.php',
