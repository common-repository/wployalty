...
class RewardPage extends Base {
	function getRewards() {
		...
		$data['data']    = array(
			...
			'limit'         => $limit,
			'edit_base_url' => admin_url( 'admin.php?' . http_build_query( array(
	...
	function bulkAction() {
		...
		$data          = apply_filters( 'wlr_before_reward_bulk_action', array(), $action_mode, $selected_list );
		if ( ! empty( $data ) ) {
			wp_send_json( $data );
		}
	...
	function deleteReward() {
		...
		if ( ! empty( $reward ) && $reward->reward_type == 'redeem_coupon' ) {
			$status                  = $reward_model->checkCampaignHaveReward( $id );
			$data['data']['message'] = sprintf( __( 'Please remove "%s" reward in campaign', 'wp-loyalty-rules' ), $reward->name );
		}
		if ( ! $status ) {
