...
class Customers extends Base {
	function getCustomerList() {
		...
		$total_count   = $user->getQueryData( $query_data, 'COUNT( DISTINCT id) as total_count', array(
			'user_email',
			'refer_code'
		), false );
	...
	function updateCustomerPointWithCommand() {
		...
		if ( ! $this->isBasicSecurityValid( 'wlr_common_user_nonce' ) || $id <= 0 || $points <= 0 || ! in_array( $point_type, array(
				'add',
				'reduce',
				'overwrite'
			) ) ) {
	...
	function getCustomerTransaction() {
		...
		$items            = $earn_transaction->getQueryData( $query_data, 'id,action_type,order_id,order_currency,order_total,points,display_name,transaction_type,customer_command,created_at', array(
			'order_id',
			'display_name'
		), true, false );
		$total_count      = $earn_transaction->getQueryData( $query_data, 'COUNT( DISTINCT id) as total_count', array(
			'order_id',
			'display_name'
		), false );
	...
	function getCustomerRewards() {
		...
		$items       = $user_reward->getQueryData( $query_data, '*', array(
			'discount_code',
			'display_name'
		), true, false );
		$total_count = $user_reward->getQueryData( $query_data, 'COUNT( DISTINCT id) as total_count', array(
			'discount_code',
			'display_name'
		), false );
		foreach ( $items as $item ) {
